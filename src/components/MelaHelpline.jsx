...
export default function MelaHelpline() {
  ...
  const guidelines = [
    {
      title: "Fast-Dial Protocol",
      text: "Only use emergency numbers for immediate life-threatening situations. Stay on the line until coordinates are confirmed.",
    },
    {
      title: "Control Room Usage",
      text: "For path updates, missing baggage, or reporting crowd bottlenecks, the Central Control Room is your primary point.",
    },
    {
        ...
        <div className="relative z-20 flex flex-col md:flex-row md:items-end justify-between w-full gap-4 text-left">
          <div className="max-w-xl">
            ...
            <p className="text-xs md:text-sm text-brand-primary-light/90 mt-2 leading-relaxed font-semibold">
              Dedicated support for a seamless and safe spiritual journey. Help
              is just a call away.
            </p>
          </div>
          ...
          <div className="relative shrink-0 w-full md:w-auto">
            ...
            <Search
              size={14}
              className="absolute left-3 top-3 text-neutral-secondary"
            />
          </div>
        </div>
      </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-neutral-bg/60 border-b border-brand-primary-border/20">
                    <th className="p-4 pl-6 text-[10px] font-extrabold text-neutral-secondary uppercase tracking-wider">
                      Department
                    </th>
                    <th className="p-4 text-[10px] font-extrabold text-neutral-secondary uppercase tracking-wider">
                      Designation
                    </th>
                    <th className="p-4 text-[10px] font-extrabold text-neutral-secondary uppercase tracking-wider">
                      Contact
                    </th>
                    <th className="p-4 pr-6 text-[10px] font-extrabold text-neutral-secondary uppercase tracking-wider text-right">
                      Action
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-neutral-bg-cool">
                  {filteredDirectory.length > 0 ? (
                    filteredDirectory.map((item, idx) => (
                      <tr
                        key={idx}
                        className="hover:bg-neutral-bg/35 transition-colors"
                      >
                        <td className="p-4 pl-6">
                          <p className="font-sans font-extrabold text-sm text-neutral-dark">
                            {item.dept}
                          </p>
                    ...
                    <tr>
                      <td
                        colSpan={4}
                        className="p-8 text-center text-xs text-neutral-secondary"
                      >
          ...
          <div className="bg-white border border-brand-primary-border/25 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-4">
              {guidelines.map((guide, idx) => (
                <div
                  key={idx}
                  className="flex flex-col gap-1.5 p-4 bg-neutral-bg border border-brand-primary-border/15 rounded-xl hover:border-brand-primary-border/30 transition-all"
                >
                  <h5 className="font-sans font-extrabold text-xs text-neutral-dark">
                    {guide.title}
                  </h5>
            ...
            <div className="bg-brand-primary-dark text-white p-4.5 rounded-xl text-left border border-brand-primary-border/10">
              ...
              <p className="text-[10px] text-brand-primary-light/85 leading-normal">
                We are here to support your devotion. Stay calm and helpful to
                others.
              </p>
            </div>
          </div>
