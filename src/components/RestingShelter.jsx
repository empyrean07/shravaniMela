...
export default function RestingShelter() {
  ...
  const shelters = [
    { id: 1, name: "Sultanganj Main Yatri Niwas", milestone: "Sultanganj", km: 1.0, lat: 25.2435, lon: 86.7312, capacity: "1500 Pilgrims", facilities: "Sanitation, Fans, Mattresses", status: "Active", ref: "Sultanganj Bypass Road" },
    { id: 2, name: "Asarganj Transit Shivir 4", milestone: "Asarganj", km: 29.5, lat: 25.1215, lon: 86.6912, capacity: "800 Pilgrims", facilities: "Medical Aid, Bathrooms, Coolers", status: "Active", ref: "KM 29.5 Crossing" },
    { id: 3, name: "Katoria Hilly Route Shelter", milestone: "Katoria", km: 68.1, lat: 24.7512, lon: 86.7112, capacity: "1200 Pilgrims", facilities: "Sand Beds, First Aid, Charging", status: "Active", ref: "Near Inaravaran Camp" },
    { id: 4, name: "Deoghar Mega Dharamshala", milestone: "Deoghar", km: 106.0, lat: 24.4819, lon: 86.7029, capacity: "5000 Pilgrims", facilities: "AC Wards, Heavy Security, CCTV", status: "Active", ref: "Baidyanath Dham Station Road" }
  ];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-neutral-secondary font-medium mt-0.5">
            Free resting facilities, shivirs and halls along the entire Yatra pathway.
          </p>
        </div>
      </div>
      ...
      <div className="bg-brand-primary-dark text-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center shadow-lg border border-brand-primary-border/10 relative overflow-hidden">
        ...
        <div className="flex-1 text-left relative z-10">
          ...
          <p className="text-amber-300 font-bold text-sm md:text-base mb-4 flex items-center gap-1.5">
            ...
            <span className="text-brand-primary-light/65 font-medium text-xs">({activeShelter.milestone} - KM {activeShelter.km})</span>
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs text-brand-primary-light mt-2 border-t border-brand-primary-border/15 pt-4">
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Shelter Capacity</span>
              <span className="text-white font-semibold">{activeShelter.capacity}</span>
            </div>
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Provided Amenities</span>
              <span className="text-white font-semibold">{activeShelter.facilities}</span>
            </div>
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Location Landmark</span>
              <span className="text-white font-semibold">{activeShelter.ref}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-brand-primary-border/15 pb-4">
          <div>
            ...
            <p className="text-xs text-neutral-secondary mt-1">
              Geographical location coordinates of the selected resting shelter/Dharamshala.
            </p>
          </div>
          <a 
      ...
      <div className="flex flex-col gap-4 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {shelters.map((shelter, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedShelter === index ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-500'}`}>
                  <Bed size={18} />
                </div>
                <div className="text-left">
                  <h5 className="font-sans font-bold text-sm text-neutral-dark">{shelter.name}</h5>
