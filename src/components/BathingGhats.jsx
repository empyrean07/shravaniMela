...
export default function BathingGhats() {
  ...
  const ghats = [
    { id: 1, name: "Jahaj Ghat (Main Bathing Area)", milestone: "Sultanganj", km: 0.2, lat: 25.2485, lon: 86.7390, flow: "Safe/Muted", guard: "24/7 SDRF Divers", status: "Active", ref: "Sultanganj Town Center" },
    { id: 2, name: "Ajgaibinath Temple Ghat", milestone: "Sultanganj", km: 0.5, lat: 25.2490, lon: 86.7410, flow: "Rocky Riverbed", guard: "SDRF + Police Post", status: "Active", ref: "Below Ajgaibinath Hill Shikhara" },
    { id: 3, name: "Sultanganj Seva Ghat", milestone: "Sultanganj", km: 0.1, lat: 25.2480, lon: 86.7375, flow: "Silted/Shallow", guard: "Volunteers + Safety Nets", status: "Active", ref: "Near Seva Samiti Tent" },
    { id: 4, name: "Sultanganj VIP Bathing Area", milestone: "Sultanganj", km: 0.0, lat: 25.2470, lon: 86.7360, flow: "Clean/Controlled", guard: "Barricades & Policemen", status: "Active", ref: "Near Administrative Office" }
  ];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-neutral-secondary font-medium mt-0.5">
            Holy Ganga water bathing zones with security fencing and rescue divers.
          </p>
        </div>
      </div>
      ...
      <div className="bg-brand-primary-dark text-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center shadow-lg border border-brand-primary-border/10 relative overflow-hidden">
        ...
        <div className="flex-1 text-left relative z-10">
          ...
          <p className="text-cyan-300 font-bold text-sm md:text-base mb-4 flex items-center gap-1.5">
            ...
            <span className="text-brand-primary-light/65 font-medium text-xs">({activeGhat.milestone} - KM {activeGhat.km})</span>
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs text-brand-primary-light mt-2 border-t border-brand-primary-border/15 pt-4">
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Water Flow</span>
              <span className="text-white font-semibold">{activeGhat.flow}</span>
            </div>
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Safety Guarding</span>
              <span className="text-white font-semibold">{activeGhat.guard}</span>
            </div>
            <div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-brand-primary-border/15 pb-4">
          <div>
            ...
            <p className="text-xs text-neutral-secondary mt-1">
              Geographical location coordinates of the selected bathing ghat in Sultanganj.
            </p>
          </div>
          <a 
      ...
      <div className="flex flex-col gap-4 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {ghats.map((ghat, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedGhat === index ? 'bg-cyan-500 text-white' : 'bg-cyan-50 text-cyan-500'}`}>
                  <Waves size={18} />
                </div>
                <div className="text-left">
                  <h5 className="font-sans font-bold text-sm text-neutral-dark">{ghat.name}</h5>
