...
import { ArrowLeft, BatteryCharging, MapPin, Compass, CheckCircle } from 'lucide-react';
...
export default function MobileCharging() {
  ...
  const kiosks = [
    { id: 1, name: "Sultanganj Shelter Charging Kiosk", milestone: "Sultanganj", km: 1.0, lat: 25.2435, lon: 86.7312, ports: "48 USB Ports", power: "Solar Powered", status: "Active", ref: "Sultanganj Yatri Niwas Hall 2" },
    { id: 2, name: "Asarganj Transit Point Kiosk", milestone: "Asarganj", km: 29.5, lat: 25.1215, lon: 86.6912, ports: "24 USB Ports", power: "Grid + Backed Up", status: "Active", ref: "Transit Shivir 4 Office" },
    { id: 3, name: "Katoria Path Kiosk 3", milestone: "Katoria", km: 68.1, lat: 24.7512, lon: 86.7112, ports: "36 USB Ports", power: "Solar Locker Cabinets", status: "Active", ref: "Inaravaran Resting Shelter" },
    { id: 4, name: "Deoghar Entry Point Locker", milestone: "Deoghar", km: 104.5, lat: 24.4925, lon: 86.7081, ports: "64 USB Ports", power: "Super-fast Charging Hub", status: "Active", ref: "Darshaniya Mod Welcome Zone" }
  ];
      ...
      <div className="bg-brand-primary-dark text-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center shadow-lg border border-brand-primary-border/10 relative overflow-hidden">
        ...
        <div className="flex-1 text-left relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs text-brand-primary-light mt-2 border-t border-brand-primary-border/15 pt-4">
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Available Ports</span>
              <span className="text-white font-semibold">{activeKiosk.ports}</span>
            </div>
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Power Source</span>
              <span className="text-white font-semibold">{activeKiosk.power}</span>
            </div>
            <div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-brand-primary-border/15 pb-4">
          <div>
            ...
            <p className="text-xs text-neutral-secondary mt-1">
              Geographical location coordinates of the selected mobile charging locker kiosk.
            </p>
          </div>
          <a 
      ...
      <div className="flex flex-col gap-4 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {kiosks.map((k, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedKiosk === index ? 'bg-orange-500 text-white' : 'bg-orange-50 text-orange-500'}`}>
                  <BatteryCharging size={18} />
                </div>
                <div className="text-left">
                  <h5 className="font-sans font-bold text-sm text-neutral-dark">{k.name}</h5>
