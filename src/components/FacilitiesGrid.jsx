...
export default function FacilitiesGrid({ isHome = false }) {
  ...
  const [lostReports, setLostReports] = useState([
    ...
    },
    {
      ...
      date: "2026-07-03",
      image:
        "https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=150&auto=format&fit=crop&q=60",
    },
  ]);
  ...
  const [foundItems, setFoundItems] = useState([
    {
      ...
      type: "Belonging",
      image:
        "https://images.unsplash.com/photo-1627124765135-566b535f9a45?w=300&auto=format&fit=crop&q=60",
    },
    {
      id: 2,
      ...
      type: "Belonging",
      image:
        "https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=300&auto=format&fit=crop&q=60",
    },
    {
      id: 3,
    ...
    },
    {
      ...
      type: "Person",
      image:
        "https://images.unsplash.com/photo-1503919545889-aef636e10ad4?w=300&auto=format&fit=crop&q=60",
    },
  ]);
  ...
  const facilities = [
    {
      id: 1,
      title: "Medical Camps",
      desc: "24/7 first-aid and ambulance service at every 2km.",
      icon: HeartPulse,
    ...
    },
    {
      ...
      desc: "Filtered water points (Pyaau) along the entire route.",
      icon: Droplets,
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {visibleFacilities.map((facility) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (facility.id === 1 || facility.title === "Medical Camps") {
                    ...
                    facility.id === 2 ||
                    facility.title.includes("Water")
                  ) {
                    navigate("/water");
                  } else if (
                    facility.id === 3 ||
                    facility.title.includes("Shelter")
                  ) {
                    navigate("/shelter");
                  } else if (
                    facility.id === 4 ||
                    facility.title.includes("Food")
                  ) {
                    navigate("/food");
                  } else if (
                    facility.id === 5 ||
                    facility.title.includes("Security")
                  ) {
                    navigate("/security");
                  } else if (
                    facility.id === 7 ||
                    facility.title.includes("Lost")
                  ) {
                    navigate("/lostfound");
                  } else if (
                    facility.id === 8 ||
                    facility.title.includes("Ghats")
                  ) {
                    navigate("/ghats");
                  } else if (
                    facility.id === 10 ||
                    facility.title.includes("Charging")
                  ) {
                    navigate("/charging");
                  } else if (
                    facility.id === 11 ||
                    facility.title.includes("Cloak")
                  ) {
                    navigate("/cloakroom");
                  } else if (
                    facility.id === 12 ||
                    facility.title.includes("Satsang")
                  ) {
                    navigate("/satsang");
                  }
                }}
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl ${facility.color} flex items-center justify-center transition-transform duration-300 group-hover:scale-105`}
                >
                  <Icon size={20} />
                </div>
                <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-neutral-secondary mt-1">
                Helping reunite pilgrims with their loved ones and belongings
                during the Shravani Mela crowd.
              </p>
            </div>
              ...
              <div className="flex-1 w-full bg-white border border-brand-primary-border/25 rounded-2xl p-6 shadow-sm">
                {!isAdminMode ? (
                  ...
                  >
                    ...
                    <div>
                      <label className="text-[10px] uppercase text-neutral-secondary font-bold block mb-1">
                        Report Category
                      </label>
                      <select
                        value={reportType}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full h-10 px-3 bg-neutral-bg border border-brand-primary-border/20 rounded-lg text-xs focus:outline-none focus:border-brand-primary"
                      >
                        <option value="Claimable at Helpdesk 1">
                          Claimable at Helpdesk 1
                        </option>
                        <option value="Claimable at Helpdesk 2">
                          Claimable at Helpdesk 2
                        </option>
                        <option value="Claimable at Helpdesk 5">
                          Claimable at Helpdesk 5
                        </option>
                        <option value="Claimable at Temple Office">
                          Claimable at Temple Office
                        </option>
                        <option value="Reunited Successfully!">
              ...
              <div className="flex-1 w-full bg-white border border-brand-primary-border/25 rounded-2xl p-6 shadow-sm flex flex-col gap-4 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-1">
                  {foundItems.map((item) => (
                    <div
                      key={item.id}
                      className="border border-brand-primary-border/15 rounded-2xl overflow-hidden flex flex-col hover:border-brand-primary-border/30 transition-all bg-neutral-bg/30 h-full"
                    >
                      {/* Image Header */}
                      <div className="w-full h-32 bg-neutral-bg-cool relative">
                        ...
                        <span
                          className={`absolute top-3 right-3 text-[8px] font-extrabold px-2 py-0.5 rounded shadow-sm uppercase ${
                            item.type === "Person"
                      ...
                      <div className="p-4 flex flex-col justify-between flex-1">
                        <div>
                          <span className="font-sans font-extrabold text-xs text-neutral-dark block mb-0.5">
                            {item.name}
                          </span>
                          <p className="text-[10px] text-neutral-secondary font-semibold">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {lostReports.length > 0 ? (
                  lostReports.map((report) => (
                    <div
                      key={report.id}
                      className="p-4 border border-brand-primary-border/10 rounded-2xl hover:border-brand-primary-border/30 transition-all flex flex-col gap-4 bg-neutral-bg/25"
                    >
                      <div className="flex items-center gap-3.5 w-full">
                        ...
                        <div className="text-left flex-1">
                          <div className="flex items-center gap-2 mb-0.5">
                            <span className="font-sans font-extrabold text-xs text-neutral-dark">
                              {report.name}
                            </span>
                            <span
                              className={`text-[8px] font-extrabold px-1.5 py-0.25 rounded uppercase ${
                                report.type === "Person"
                          ...
                          <p className="text-[10px] text-neutral-secondary font-semibold">
                            Last Location: {report.location} | Date:{" "}
                            {report.date}
                          </p>
                          {report.desc && (
                            <p className="text-[10px] text-neutral-secondary mt-1.5 font-medium leading-relaxed">
                              {report.desc}
                            </p>
                          )}
                        </div>
                      ...
                      <div className="border-t border-neutral-bg-cool pt-3 flex justify-between items-center w-full">
                        <div>
                          <p className="text-[9px] uppercase text-neutral-secondary font-bold leading-none">
                            Contact Person
                          </p>
                          <a
                            href={`tel:${report.contact}`}
                            className="text-xs font-mono font-bold text-brand-primary hover:underline"
                          >
                            {report.contact}
                          </a>
                        </div>
                  ...
                  <p className="text-xs text-neutral-secondary p-4 col-span-2 text-center">
                    No lost reports filed yet.
                  </p>
                )}
              </div>
            </div>
