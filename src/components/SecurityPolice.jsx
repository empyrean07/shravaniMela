...
export default function SecurityPolice() {
  ...
  const policePosts = [
    { id: 1, name: "Sultanganj Station Security Post", milestone: "Sultanganj", km: 0.1, lat: 25.2420, lon: 86.7320, staff: "45 Officers", type: "Active Patrol", status: "Optimal", ref: "Sultanganj Railway Station" },
    { id: 2, name: "Asarganj Highway Checkpost", milestone: "Asarganj", km: 26.0, lat: 25.1481, lon: 86.6841, staff: "20 Officers", type: "Vehicle Check", status: "Optimal", ref: "Asarganj Camp Crossing" },
    { id: 3, name: "Katoria Sand Path Patrol Unit", milestone: "Katoria", km: 64.0, lat: 24.8100, lon: 86.7210, staff: "30 Patrols", type: "Foot Patrol", status: "Optimal", ref: "KM 64 Hilly pathway" },
    { id: 4, name: "Deoghar Temple Main Control Room", milestone: "Deoghar", km: 106.0, lat: 24.4819, lon: 86.7029, staff: "120 Officers", type: "CCTV Control Room", status: "Optimal", ref: "Near Temple Gate 1" }
  ];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-neutral-secondary font-medium mt-0.5">
            24/7 security surveillance, queue control and active police assistance.
          </p>
        </div>
      </div>
      ...
      <div className="bg-brand-primary-dark text-white rounded-2xl p-6 md:p-8 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center shadow-lg border border-brand-primary-border/10 relative overflow-hidden">
        ...
        <div className="flex-1 text-left relative z-10">
          ...
          <p className="text-indigo-300 font-bold text-sm md:text-base mb-4 flex items-center gap-1.5">
            ...
            <span className="text-brand-primary-light/65 font-medium text-xs">({activePost.milestone} - KM {activePost.km})</span>
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs text-brand-primary-light mt-2 border-t border-brand-primary-border/15 pt-4">
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Force Strength</span>
              <span className="text-white font-semibold">{activePost.staff}</span>
            </div>
            <div>
              <span className="text-[10px] uppercase text-brand-primary-light/50 font-bold block mb-0.5">Operations Type</span>
              <span className="text-white font-semibold">{activePost.type}</span>
            </div>
            <div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-brand-primary-border/15 pb-4">
          <div>
            ...
            <p className="text-xs text-neutral-secondary mt-1">
              Geographical location coordinates of the selected security checkpoint / patrol unit.
            </p>
          </div>
          <a 
      ...
      <div className="flex flex-col gap-4 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {policePosts.map((post, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedPost === index ? 'bg-indigo-500 text-white' : 'bg-indigo-50 text-indigo-500'}`}>
                  <Shield size={18} />
                </div>
                <div className="text-left">
                  <h5 className="font-sans font-bold text-sm text-neutral-dark">{post.name}</h5>
